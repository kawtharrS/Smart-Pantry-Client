...
const IngredientsModalContent = ({ title, description, ingredients }: { title: string; description: string; ingredients: RecipeIngredient[] }) => (
        ...
        <div className="space-y-2">
          {ingredients.map(ingredient => (
            <div key={ingredient.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <span className="text-gray-600">{ingredient.caloriesPer100g} cal/100g</span>
            </div>
          ))}
        </div>
      )}
...
const InstructionsModalContent = ({ title, instructions }: { title: string; instructions: RecipeInstruction[] }) => (
  <div className="space-y-6">
    <div>
      <p className="text-sm font-medium text-gray-500 mb-1">Recipe Name</p>
      <p className="text-lg font-semibold text-gray-900">{title}</p>
...
const Modal = ({ isOpen, onClose, type, children }: { isOpen: boolean; onClose: (e: React.MouseEvent) => void; type: 'ingredients' | 'instructions'; children: React.ReactNode }) => {
...
const CardRecipe = ({ id, title, description, prep_time_min, cook_time_min, serving, textColor = "text-amber-600", ingredients: propsIngredients }: CardRecipeProps) => {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    if (!target.closest('button')) window.location.href = `/recipe/${id}`;
  };
  ...
  const renderModalContent = () => modalType === 'ingredients' 
    ? <IngredientsModalContent title={title} description={description} ingredients={ingredients} />
    : <InstructionsModalContent title={title} instructions={instructions} />;

  return (
    <>
      <div className="h-full">
        <div className="relative bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden h-full flex flex-col cursor-pointer" onClick={handleCardClick}>
          <div className="p-5 flex-1 flex flex-col">
            <h3 className={`text-xl font-bold ${textColor} mb-2 line-clamp-2`}>{title}</h3>
            ...
            <div className="grid grid-cols-3 gap-3 mb-3 py-3 border-t border-gray-200">
              <div className="text-center"><p className="text-base font-bold text-red-400">{prep_time_min}min</p><p className="text-xs text-gray-500 mt-0.5">Prep Time</p></div>
              <div className="text-center"><p className="text-base font-bold text-green-500">{cook_time_min}min</p><p className="text-xs text-gray-500 mt-0.5">Cook Time</p></div>
              <div className="text-center"><p className="text-base font-bold text-blue-400">{serving}/person</p><p className="text-xs text-gray-500 mt-0.5">Serving</p></div>
            </div>

            <div className="flex flex-col w-full mt-auto pt-2 gap-2">
              <button onClick={openModal('ingredients')} className="w-full !bg-amber-500 hover:bg-amber-700 text-white font-semibold py-2 rounded-lg transition-colors duration-200 text-sm text-center">Ingredients</button>
              <button onClick={openModal('instructions')} className="w-full !bg-gray-400 hover:bg-green-800 text-white font-semibold py-2 rounded-lg transition-colors duration-200 text-sm text-center">Instructions</button>
            </div>
          </div>
        </div>
      </div>
